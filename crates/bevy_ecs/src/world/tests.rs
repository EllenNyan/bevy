...
fn relation_query_raw(storage_type: StorageType) {
    ...
    query
        .new_s(&world, TargetFilter::<ChildOf>::new().target(parent1))
        .apply_filters();
    let mut iter = query.iter_mut(&mut world);
    assert!(iter.next().unwrap().0 == child1);
    assert!(matches!(iter.next(), None));
    ...
    query
        .new_s(&world, TargetFilter::<ChildOf>::new().target(parent2))
        .apply_filters();
    let mut iter = query.iter_mut(&mut world);
    assert!(iter.next().unwrap().0 == child2);
    assert!(matches!(iter.next(), None));
...
fn relation_query_mut_raw(storage_type: StorageType) {
    ...
    query
        .new_s(&world, TargetFilter::<MyRelation>::new().target(target2))
        .apply_filters();
...
fn explicit_path() {
    ...
    query
        .new_s::<u32, InData<InTuple<_, 0>>>(&world, TargetFilter::new().target(target))
        .apply_filters();
}
...
fn foo() {
    ...
    query
        .new_s(&world, TargetFilter::<u32>::new().target(target))
        .apply_filters()
        .for_each(&world, |_| ())
}
...
fn without_filter_raw(storage_type: StorageType) {
    ...
    let data = world
        .query_filtered::<(Entity, &String), Without<Relation<MyRelation>>>()
        .new_s(&world, TargetFilter::<MyRelation>::new().target(target1))
        .apply_filters()
        .iter(&world)
        .collect::<Vec<(Entity, &String)>>();
    assert_eq!(
...
#[test]
fn duplicated_s() {
    duplicated_s_raw(StorageType::SparseSet);
    duplicated_s_raw(StorageType::Table);
}

fn duplicated_s_raw(storage_type: StorageType) {
    let mut world = World::new();
...
fn with_filter_raw(storage_type: StorageType) {
    ...
    let [e1, e2]: [Entity; 2] = q
        .new_s(&world, TargetFilter::<u32>::new().target(target1))
        .apply_filters()
        .iter(&world)
        .collect::<Vec<_>>()
        .try_into()
    ...
    let [e1, e2]: [Entity; 2] = q
        .new_s(&world, TargetFilter::<u32>::new().target(target2))
        .apply_filters()
        .iter(&world)
        .collect::<Vec<_>>()
        .try_into()
    ...
    assert_eq!(e2, many_relations);
    let []: [Entity; 0] = q
        .new_s(&world, TargetFilter::<u32>::new().target(no_relation))
        .apply_filters()
        .iter(&world)
        .collect::<Vec<_>>()
        .try_into()
