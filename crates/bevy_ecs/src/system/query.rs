    ...
    F::Fetch: FilterFetch,
{
    ...
    fn set_s(&mut self, target_filter: QueryTargetFilters<Q, F>) -> &mut Self {
        self.state.set_s(&self.world, target_filter);
        self
    }
}
...
{
    fn drop(&mut self) {
        self.set_s(QueryTargetFilters::new());
    }
}
    ...
    F::Fetch: FilterFetch,
{
    ...
    pub fn apply_filters(self) -> &'a mut Query<'b, Q, F> {
        ...
        query.state.set_s(query.world, filters);
        query
    }
}
